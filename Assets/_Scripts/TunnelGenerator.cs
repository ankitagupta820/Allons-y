...
public class TunnelGenerator : MonoBehaviour
{
    ...
    [Tooltip("If you've mannually added a gameobject as an initial reference, put that object here")]
    ...
    private void Start()
    {
        ...
        if (presetObject != null) prevGeneratedObj = presetObject;
    }
        ...
        for (int i = 0; i < quantity; i++)
        {
            ...
            if (newObj != null)
            {
                if (locateBasedOnPrevObj && prevGeneratedObj != null)
                {
                    ...
                    if (fix2DLoc)
                    {
                        Vector3 newObjLoc = new Vector3(prevGeneratedObj.transform.position.x,
                        Random.Range(minVerticalDistance, maxVerticalDistance) + prevGeneratedObj.transform.position.y,
                        prevGeneratedObj.transform.position.z
                        );
